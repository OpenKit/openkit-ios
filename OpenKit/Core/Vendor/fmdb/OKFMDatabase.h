...
#import "OKFMDatabasePool.h"
    ...
    #define OKFMDBAutorelease(__v) ([__v autorelease]);
    #define OKFMDBReturnAutoreleased OKFMDBAutorelease

    #define OKFMDBRetain(__v) ([__v retain]);
    #define OKFMDBReturnRetained OKFMDBRetain

    #define OKFMDBRelease(__v) ([__v release]);

	#define OKFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define OKFMDBAutorelease(__v)
    #define OKFMDBReturnAutoreleased(__v) (__v)

    #define OKFMDBRetain(__v)
    #define OKFMDBReturnRetained(__v) (__v)

    #define OKFMDBRelease(__v)

	#if TARGET_OS_IPHONE
		// Compiling for iOS
		#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 60000
			// iOS 6.0 or later
			#define OKFMDBDispatchQueueRelease(__v)
		#else
			// iOS 5.X or earlier
			#define OKFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
		#endif
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 ### Usage
 The three main classes in OKFMDB are:

 - `OKFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<OKFMResultSet>` - Represents the results of executing a query on an `OKFMDatabase`.
 - `<OKFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<OKFMDatabasePool>` - A pool of `OKFMDatabase` objects.
 - `<OKFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [OKFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [OKFMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `OKFMDatabase` object and use it across multiple threads. Instead, use `<OKFMDatabaseQueue>`.

 */

@interface OKFMDatabase : NSObject  {
...
/** Create a `OKFMDatabase` object.
 
 An `OKFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `OKFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `OKFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    OKFMDatabase *db = [OKFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    OKFMDatabase *db     = [OKFMDatabase databaseWithPath:dbPath];
 ...
 @return `OKFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `OKFMDatabase` object.
...
/** Execute select statement
 ...
 Executing queries returns an `<OKFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[OKFMResultSet next]>`) from one record to the other.
 ...
 @return A `<OKFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see OKFMResultSet
 @see [`OKFMResultSet next`](<[OKFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (OKFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (OKFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (OKFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (OKFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (OKFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(OKFMDatabase *adb) {
        ...
        int rowCount = 0;
        OKFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        OKFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [OKFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one OKFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using OKFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to OKFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using OKFMDB you will not need to interact directly with `OKFMStatement`, but rather with `<OKFMDatabase>` and `<OKFMResultSet>` only.
...
@interface OKFMStatement : NSObject {
